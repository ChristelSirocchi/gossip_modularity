...
# distance selection
def community_selection(self):
    return np.random.choice(self.neighbours, 1, p = self.comm_p)[0]
...
def comm_mean(self, neigh):
    ...
    self.value = new_value
    # share new label
    self.comm[neigh.unique_id] = neigh.label
    neigh.comm[self.unique_id] = self.label
    self.update_label()
    neigh.update_label()
    # return updated nodes
    return self, neigh
