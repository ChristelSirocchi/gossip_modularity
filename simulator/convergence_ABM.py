...
class Agent():
    ...
    def __init__(self, unique_id, model, features, select_neighbour, interact, next_move_time, event_logger):
        ...
        # get neighbours community
        self.comm = {n : n for n in self.neighbours}
        # get neighbours degree
        self.degree_p = self.get_degrees()
        # get neighbours distance
        self.distance_p = self.get_distances()
        # get neighbours distance
        self.comm_p = self.get_communities()

    
    def update_label(self):
        # calculate label frequency 
        freqs = Counter(self.comm.values())
        # find the size of the largest coloring
        max_freq = max(freqs.values())
        # find the most frequent colorings
        high_labels = {label for label, freq in freqs.items() if freq == max_freq}
        # if there is a unique maximum coloring
        if len(high_labels) == 1:
            self.label = high_labels.pop()
        elif len(high_labels) > 1:
            # Prec-Max
            if self.label not in high_labels:
                self.label = max(high_labels)
    ...
    def get_communities(self):
        # get coloring frequencies
        freqs = Counter(self.comm.values())
        # sum of the inverse of frequencies
        #tot = sum([1/s for s in freqs.values()])
        # probabilities per coloring
        #probs = {label: 1/(freq*tot) for label, freq in freqs.items()}
        # probabilities for nodes
        #self.comm_p = [probs[group]/freqs[group] for node, group in self.comm.items()]
        self.comm_p = [1/(len(freqs)*freqs[group]) for node, group in self.comm.items()]
        return self.comm_p
...
class NetworkModel():
    ...
    # method to retrieve label logs as pandas dataframe
    def get_label_log(self):   
        df = pd.DataFrame(self.label_log, index = ["Agent_"+ str(i) for i in range(self.G.number_of_nodes())]).T
        return df
...
class EventDrivenLogger(Logger):
    ...
    def run(self):
        while True:
            ...
            # get label logs
            self.model.label_log[self.env.now] = self.model.get_labels()
            # pause
