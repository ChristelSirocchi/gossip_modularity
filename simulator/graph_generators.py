...
def get_connected_config(deg_list):
    while True:
        G = nx.configuration_model(deg_list)
        if nx.is_connected(G):
            break
    return G

def get_connected_sbm(comm_size, sbm_p):
    while True:
        G = nx.stochastic_block_model(comm_size, sbm_p)
        if nx.is_connected(G):
            break
    return G
...
def get_connected_community(s,k,m,beta,alpha,gamma):
    while True:
        G, node_communities = generate(farz_params={"n":s, #number of nodes
             "k":k, #number of communities
             "m":m, # link density
             "beta":beta, #strength of the community [0.75,0.95]
             "alpha":alpha, #transitivity [0.2,0.8]
             "gamma":gamma, #degree correlation [0.2,0.8]
             ...
             'directed':False, 
             'weighted':False})     
        node_comm = np.array(list(node_communities.values())).flatten()
        if nx.is_connected(G):
            break
    return G, node_comm
